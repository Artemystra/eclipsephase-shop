...
export async function withTempActor(data, fn) {
  ...
  try {
    ...
    if ((CONFIG.Actor.documentClass.MANAGED_TYPES ?? []).includes(actor.type)) {
 ...
 * Without this a single roll outlasts Mocha's timeout on any client with Dice So Nice active.
 * @param {Function} fn - Called with no arguments; its return value is awaited and passed through
 * @returns {Promise<*>} Whatever fn returned
 */
export async function withoutDiceAnimation(fn) {
  suppressDice3d();
  try {
    return await fn();
  } finally {
