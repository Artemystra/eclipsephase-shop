...
const frozen = JSON.parse(fs.readFileSync(path.join(ROOT, "test", "fixtures", "schema", "shop-template-2.3.json"), "utf8"));

describe("the shop data model", () => {
  test("its defaults match the schema template.json carried before the model existed", () => {
    expect(ShopModel.cleanData()).toEqual({ ...frozen, schemaVersion: 1 });
  });

  test("a legacy shop never cleans to itself, or the type migration's replacement is dropped", () => {
    expect(ShopModel.cleanData(frozen)).not.toEqual(frozen);
    expect(ShopModel.cleanData(frozen).schemaVersion).toEqual(1);
  });
  ...
  test("the module declares the shop as its own document sub-type", () => {
    const manifest = JSON.parse(fs.readFileSync(path.join(ROOT, "module.json"), "utf8"));
    expect(manifest.documentTypes.Actor.shop).toEqual({});
  });
});
